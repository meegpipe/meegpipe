...
%
%  23.08.2013 - (German Gomez-Herreo) use plot2svg.copyfile instead of builtin copyfile 
%  14.11.2012 - (German Gomez-Herrero) Quick and dirty bugfix in latex2svg
%  09.11.2012 - (German Gomez-Herrero) Fixed bug in projection()
%  23.08.2012 - (German Gomez-Herrero) Added svgpan support
...
% Copy SVGPan.js to the output directory (Added by GGH)
tmpPathName = fileparts(finalname);
if isempty(tmpPathName),
    tmpPathName = pwd;
end
SVGfile = fullfile(fileparts(mfilename('fullpath')), 'SVGPan.js');
copyfile(SVGfile, tmpPathName);
...
fprintf(fid, '<script xlink:href="SVGPan.js"/>\n');
fprintf(fid, '<g id="viewport">\n');
...
try
    ...
    % GGH: Quick and dirty fix for \fontsize{\d\d}, which appear e.g. when
    % plotting eegplot figures with events
    
    StringText = regexprep(StringText, '\\fontsize{\d+}', '');
