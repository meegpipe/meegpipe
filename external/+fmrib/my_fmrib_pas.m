% function ecgout=my_fmrib_pas(eegin,fs,Peaks,method)
% an implementation of the pas removal algorithm from Niazy et al, but then
% working a single vector instead of an EEG eeglab struct.
% method = 'obs','median','mean' or 'gmean'
% npc = 3;
...
if nargin < 5 || isempty(npc),
    npc=3; % includes the mean, so apart from the mean, 2 more waveforms.
end
% quick fix for rest of this code.
EEG.data=reshape(eegin,1,numel(eegin));
EEG.srate=fs;
