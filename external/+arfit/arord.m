function [sbc, fpe, aic, logdp, np] = arord(R, m, mcor, ne, pmin, pmax)
%ARORD	Evaluates criteria for selecting the order of an AR model.
%
%  [SBC,FPE,AIC]=ARORD(R,m,mcor,ne,pmin,pmax) returns approximate values
%  of the order selection criteria SBC, FPE and AIC for models of order
...
% arord.m included in ARfit. For the latest release of ARFit please visit
...
%
% This modified version returns also the AIC criterion
%
% -------------------------------------------------------------------------
...
%          tapio@gps.caltech.edu
  ...
  for p = pmax:-1:pmin
   ...
   % ----------------------------------------------------------------------
   % This line added by German Gomez-Herrero
   % logarithm of Akaike's Information Criterion (AIC)
   aic(i) =  logdp(i)/m - 2 * (ne-np(i))/ne;
   % ----------------------------------------------------------------------
