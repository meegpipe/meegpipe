...
% note: modified by GGH to support argument 'visible'
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.visible;          catch, g.visible = 'on'; end; % ADDED BY GGH
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
               ...
               'mocap' 'selectcommand' 'ctrlselectcommand' 'datastd' ...
               'normed' 'envelope' 'visible'},;
...
u(22) = uicontrol('Parent',figh, ...
  ...
  % Modified by germang.gomezherrero@gmail.com
  % The figure is now deleted after the user-provided command has run. This
  % allows for the user-provided code to access data stored in the figure
  % handle.
